...
coords_proposal_35nm <- sf::st_coordinates(proposal_35nm) |> 
  as_tibble() |> 
  mutate(proposal = "Scenario 3: Connecting rectangle (35nm)") |> 
  distinct()

coords_proposal_30nm <- sf::st_coordinates(proposal_30nm) |> 
  as_tibble() |> 
  mutate(proposal = "Scenario 2: Connecting rectangle (30nm)") |> 
  distinct()

coords_proposal_buffer_bokak <- sf::st_coordinates(bokak_buffer_35nm) |> 
  as_tibble() |> 
  mutate(proposal = "Scenario 1: Square buffers (35 nm) - Bokak") |> 
  distinct()

coords_proposal_buffer_bikar <- sf::st_coordinates(bikar_buffer_35nm) |> 
  as_tibble() |> 
  mutate(proposal = "Scenario 1: Square buffers (35 nm) - Bikar")|> 
  distinct()
...
tbl <- bind_rows(coords_proposal_buffer_bikar, 
  ...
  gt::cols_label(
    ...
    lat = "Latitude",
    proposal = "Scenario") |> 
  gt::tab_header(title = "Coordinates of proposed MPA boundaries")
...
sf::st_write(bikar_bokak_proposals, file.path(exp_path, "data/primary/processed/proposals/all_proposals.gpkg"), append = F) 
