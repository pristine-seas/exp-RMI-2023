...
```{r eval = TRUE, include = T}
exp_meta$edna |> 
  distinct(location, ps_station_id, date, collection_time, lat, lon, depth_m, n_replicates, team_lead) |> 
  gt::gt(rowname_col = "location") |> 
  gt::tab_header(title = "eDNA sampling locations") |> 
  gt::opt_interactive(use_compact_mode = T)
```
...
```{r eval = TRUE, include = T}
exp_meta$bruvs |> 
  distinct(location, ps_station_id, date, local_time, lat, lon, depth_m, team_lead) |> 
  gt::gt(rowname_col = "location") |> 
  gt::tab_header(title = "Benthic BRUVS deployment locations") |> 
  gt::opt_interactive(use_compact_mode = T)
```
...
pelagics_meta_sf |> 
  filter(location == "Bikar") |> 
  st_jitter(factor = 0.001) |> 
  mapview::mapview(zcol = c("location"), 
                   legend = TRUE,
                   col.regions = paletteer_d("ggthemes::Tableau_10", 3),
                   map.types = c("Esri.WorldImagery"),
                   layer.name = "Location",
...
```{r eval = TRUE, include = T}
exp_meta$pelagics |> 
  group_by(location, ps_station_id, date,  depth_m, team_lead) |> 
  summarize(local_time = first(time_in), 
            lat = first(lat_in), 
            lon = first(lon_out)) |> 
  ungroup() |> 
  select(location, ps_station_id, date, local_time, lat, lon,  depth_m, team_lead) |> 
  gt::gt() |> 
  gt::tab_header(title = "Benthic BRUVS deployment locations") |> 
  gt::opt_interactive(use_compact_mode = T) |> 
  gt::tab_footnote(footnote = "coordinates and time of the first rig")
```
...
```{r eval = TRUE, include = T}
exp_meta$dscm |> 
  select(location, ps_station_id, date = date_in, local_time = time_in, 
         lat = lat_in, lon = lon_in,  recording_time, depth_m, team_lead) |> 
  gt::gt() |> 
  gt::tab_header(title = "Deep sea camera deployment locations") |> 
  gt::opt_interactive(use_compact_mode = T) 
```
...
```{r eval = TRUE, include = T}
exp_meta$sub |> 
  select(location, ps_station_id, date, local_time = dive_start_time, 
         lat = surface_start_lat, lon = surface_start_lon, max_depth_m, observer_1, observer_2,
         team_lead = pilot) |> 
  gt::gt() |> 
  gt::tab_header(title = "Deep sea camera deployment locations") |> 
  gt::opt_interactive(use_compact_mode = T) 
...
birds_meta_sf |>  
  ...
  mapview::mapview(zcol = c("location"), 
                   legend = TRUE,
                   #col.regions = c(paletteer_c("grDevices::Viridis", 5),"white"),
                   map.types = c("Esri.WorldImagery"),
                   layer.name = "location",
...
```{r eval = TRUE, include = T}
exp_meta$birds |> 
  select(location, ps_station_id, date, local_time = time_start, 
         lat = lat_start, lon = lon_start, distance_km, duration_mins, team_lead) |> 
  gt::gt() |> 
  gt::tab_header(title = "Deep sea camera deployment locations") |> 
  gt::opt_interactive(use_compact_mode = T) 
```
...
```{r eval = TRUE, include = T}
exp_meta$vegetation |> 
  select(location, ps_station_id, survey_type,
         lat, lon , plot_size_m2 = size_m, team_lead) |> 
  gt::gt() |> 
  gt::tab_header(title = "Vegetation survey locations") |> 
  gt::opt_interactive(use_compact_mode = T) 
```
...
exp_meta$traverses |> 
  select(-expedition) |> 
  gt::gt() |> 
  gt::tab_header(title = "Traverse locations") |> 
  gt::opt_interactive(use_compact_mode = T) 
```
...
```{r eval = TRUE, include = T}
exp_meta$plastics |> 
  select(location, ps_station_id, date, local_time = time, lat, lon, team_lead) |> 
  gt::gt() |> 
  gt::tab_header(title = "Plastic sampling locations") |> 
  gt::opt_interactive(use_compact_mode = T) 
```
